...
\subsection{Warstwa ViewModel: Enkapsulacja Stanu}

Klasa \texttt{ViewModel} pełni rolę zarządcy stanu. Kluczowym aspektem implementacji jest tutaj wzorzec \textbf{Backing Property}, który zapewnia pełną enkapsulację danych.
...
\subsection{Warstwa Widoku: Delegacja Odpowiedzialności}
...
\subsection{Mechanizm działania: ViewModelStore a Singleton}
...
\subsection{Separacja: Kelner a Kucharz}
...
\subsection{Interfejs: Kontrakt między warstwami}
...
\subsection{Dlaczego to robimy? Praktyczne zastosowania}
...
\texttt{val viewModel: WordViewModel = viewModel()}, aplikacja wyrzuci błąd \texttt{RuntimeException}.

Funkcja biblioteczna \texttt{viewModel()} domyślnie potrafi tworzyć tylko obiekty posiadające pusty konstruktor (bez parametrów). Działa ona na zasadzie refleksji - próbuje utworzyć instancję klasy, ale nie ma pojęcia, skąd wziąć obiekt \texttt{WordRepository}, którego wymaga nasz konstruktor.

\subsection{Metafora: Samochód z placu vs Zamówienie Specjalne}
...
W Androidzie tą specyfikacją jest wzorzec \textbf{Factory}.

\subsection{Rozwiązanie: ViewModelProvider.Factory}
...
\subsection{Użycie w Composable}
