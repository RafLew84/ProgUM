\chapter{Wprowadzenie do wielowatkowosci i coroutines (Wykład 2)}

\section{Problem Zamrożonego UI i \textit{Piekło Callbacków} - Wątek Główny}
...
System operacyjny Android szybko \textit{zauważa}, że nasza aplikacja nie odpowiada na zdarzenia. Po kilku sekundach wyświetla komunikat: \textit{Aplikacja nie odpowiada} (ANR - Application Not Responding)(Rys. \ref{fig:figura-anr}).

\begin{figure}[htbp]
    \includegraphics[width=0.7\textwidth]{fig/pum2-w02-fig2.png}
    \caption{Błąd ANR.}
...
\texttt{suspend fun} ma się tak do korutyny, jak klasa ma się do instancji tej klasy; możemy mieć wiele instancji tej samej klasy:
\begin{itemize}
...
\begin{figure}[htbp]
    \includegraphics[width=1.0\textwidth]{fig/pum2-w02-coroutine_scope.png}
    \caption{Coroutines.}
    \label{fig:figura-coroutine-scope}
\end{figure}
...
\begin{itemize}
\item \textbf{Zasada Anulowania (W dół)}: Jeśli anulujesz Rodzica (np. zamkniesz ekran i anulujesz \texttt{coroutineScope}), wszystkie \textit{dzieci} są automatycznie anulowane (Canceled). Szef Zmiany mówi: \textit{Zamykamy kuchnię!}, więc wszyscy kucharze natychmiast przestają gotować swoje dania. Nikt nie zostaje w pracy po godzinach.
...
\begin{figure}[htbp]
    \includegraphics[width=1.0\textwidth]{fig/pum2-w02-blocking_ui.png}
    \caption{Blokowanie wątku ui.}
    \label{fig:figura-blocking-ui}
\end{figure}
