...
To jedno z najczęstszych pytań rekrutacyjnych dotyczących korutyn. Różnica jest fundamentalna i dotyczy modelu dystrybucji danych.
...
\begin{figure}[htbp]
    \includegraphics[width=0.7\textwidth]{fig/pum2-w07-channelvssharedflow.png}
    \caption{\texttt{Channel} vs \texttt{SharedFlow}.}
    \label{fig:Figura channelvssharedflow}
\end{figure}
...
W przeciwieństwie do \texttt{Flow}, który definiujemy deklaratywnie, obsługa Kanału przypomina obsługę tradycyjnej kolejki, ale z jedną kluczową różnicą: jest ona textbf{asynchroniczna}.
...
Dzięki temu możemy przekazać odbiorcy tylko interfejs \texttt{ReceiveChannel}, mając pewność, że przez pomyłkę nic nie wyśle ani nie zamknie kanału.

Dwie główne metody obsługi kanału to \texttt{send(element)} oraz \texttt{receive()}. Obie są funkcjami zawieszającymi (\texttt{suspend}).
...
Dlatego w praktyce rzadko używamy surowego \texttt{receive()}. Zdecydowanie lepiej używać pętli \texttt{for (item in channel)}, która automatycznie obsługuje zamknięcie kanału i bezpiecznie wychodzi z pętli.
...
\begin{figure}[htbp]
    \includegraphics[width=0.7\textwidth]{fig/pum2-w07-buffors.png}
    \caption{Typy buforów.}
    \label{fig:Figura buffors}
\end{figure}
...
W standardowym kodzie asynchronicznym (używając \texttt{await} lub \texttt{receive}) czekamy sekwencyjnie: najpierw na wynik A, potem na wynik B.
Co jednak, jeśli mamy dwa niezależne źródła danych i chcemy przetworzyć wynik z tego, które odpowie \textbf{szybciej}?

Tu z pomocą przychodzi wyrażenie \texttt{select}. Pozwala ono \textit{nasłuchiwać} na wielu kanałach (lub obiektach \texttt{Deferred}) jednocześnie i uruchomić kod skojarzony tylko z tym zdarzeniem, które nastąpi jako pierwsze.
