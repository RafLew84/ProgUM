...
System operacyjny Android szybko zauważa, że nasza aplikacja nie odpowiada na zdarzenia. Po kilku sekundach wyświetla komunikat: \textit{Aplikacja nie odpowiada} (ANR - Application Non Responding)(\ref{fig:figura-anr}).
...
Musimy wyjaśnienić czym jest przepis w naszej analogi; jest to byt, który naywamy funkcją z możliwością zawieszenia wykonania (\texttt{suspend fun})
...
Kiedy piszesz w kodzie \texttt{scope.launch \{... \}} (zaraz wyjaśnimy czym jest \texttt{scope}), ten blok kodu \texttt{launch} (nazywany konstruktorem korutyny) tworzy i uruchamia nową korutynę (nową \textit{instancję} pracy). Ta nowa, żywa korutyna zaczyna wykonywać kod wewnątrz bloku \texttt{\{...\}}. Może działać nastęująco:
\begin{enumerate}
...
W tej analogii kucharz będzie naszym \textbf{wątkiem UI} (\textbf{wątkiem głównym}), którego nie chcemy blokować długimi operacjami. Ale nie jest on jedynym pracownikiem kuchni, są również \textbf{pomocnicy kuchennego} (\textbf{wątki robocze/poboczne}). Nasza analogia wygląda teraz następująco:
...
\section{Rozwiążanie problemu ANR}
