...
W starszych wersjach Androida istniały inne tryby, które obecnie zostały uznane za niebezpieczne i oznaczone jako \texttt{deprecated} (przestarzałe):

\begin{itemize}
    \item \textbf{\texttt{MODE\_WORLD\_READABLE} (1):} Pozwalał dowolnej innej aplikacji na telefonie odczytać zawartość Twoich preferencji. Usunięty ze względów bezpieczeństwa (od Androida 7.0 rzuca wyjątek \texttt{SecurityException}).
    \item \textbf{\texttt{MODE\_WORLD\_WRITEABLE} (2):} Pozwalał innym aplikacjom na zapisywanie danych do Twojego pliku. Bardzo niebezpieczny, usunięty podobnie jak powyższy.
    \item \textbf{\texttt{MODE\_MULTI\_PROCESS} (4):} Teoretycznie pozwalał na modyfikację preferencji przez wiele procesów tej samej aplikacji. W praktyce nigdy nie działał poprawnie (nie gwarantował spójności danych) i również został wycofany.
\end{itemize}
...
Posiadając warstwę repozytorium (\texttt{UserPreferencesRepository}), która wystawia dane w postaci strumienia \texttt{Flow}, możemy przystąpić do integracji z warstwą prezentacji.

Kluczowym wyzwaniem jest tutaj konwersja \textit{zimnego} strumienia z DataStore na \textit{gorący} stan UI, dostępny natychmiast dla widoku. Wykorzystamy do tego operator \texttt{stateIn}.
...
\begin{enumerate}
    ...
    \item \textbf{DataStore (Callback):} Wykrywa zmianę w pliku i emituje nową wartość do strumienia `userNameFlow`.
